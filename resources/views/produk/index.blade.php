...
@section('content')
<div class="w-full bg-gray-50/50 min-h-screen pb-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div data-aos="fade-right">
                <span class="inline-block px-4 py-1.5 bg-green-100 text-green-700 text-xs font-bold rounded-full mb-4 uppercase tracking-widest">
                    Koleksi Produk
                </span>
        ...
        <div class="bg-white p-4 rounded-[2rem] border border-gray-100 shadow-sm mb-10" data-aos="fade-up">
            <form action="{{ route('produk.index') }}" method="GET" class="flex flex-wrap gap-4 items-center">
                <div class="relative flex-1 min-w-[300px]">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari benih, selada, atau nutrisi..." 
                        class="w-full pl-12 pr-4 py-3.5 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-green-500 transition outline-none text-sm">
                </div>

                <div class="flex items-center gap-2">
                    <select name="category" onchange="this.form.submit()" 
                            class="bg-gray-50 border-none rounded-xl px-6 py-3.5 text-sm font-bold text-gray-600 outline-none focus:ring-2 focus:ring-green-500">
                    ...
                    @if(request('search') || request('category'))
                        <a href="{{ route('produk.index') }}" class="p-3.5 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-sm">
        ...
        <div id="loading-trigger" class="py-16 text-center opacity-0 transition-opacity duration-300">
